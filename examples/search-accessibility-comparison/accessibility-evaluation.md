...
Historical note: this is saved comparison evidence from an earlier accessibility-focused MCP pass. It is useful for reviewing the shape of the accessibility checks, but it is not current release acceptance proof for any JudgmentKit MCP release or for the active design-system provenance gate.
