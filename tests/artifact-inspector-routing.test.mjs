...
for (const [exclusionId, conflictingStatement] of EXCLUSION_CASES) {
  ...
  assert.deepEqual(review.interaction_implications, {});
  assert.deepEqual(review.disclosure_implications, {});
  assert.deepEqual(review.frontend_posture, {});
...
{
  const selectedCaseDetailArtifact = `
    A release lead works through a review queue that is the primary work surface.
    For the selected case, one rendered interface is central inside the
    selected-case detail so the lead can inspect that case. They compare case
    evidence, approve or return each case, and leave an audit receipt.
  `;
  const review = recommendSurfaceTypes(selectedCaseDetailArtifact);

  assert.equal(review.recommended_surface_type, "workbench");
  assert.equal(review.evidence.artifact_inspector, undefined);
  assert.equal(
    review.interaction_implications.primary_structure,
    "Item selection, detail evidence, decision controls, and completion state.",
    "An interface that is central only within selected-case detail must not displace the explicitly primary queue.",
  );
}

{
  const selectedCaseDetailPaneArtifact = `
    A release lead works through a review queue that is the primary work surface.
    Within the detail pane for the selected case, one rendered interface remains
    central so the lead can inspect that case. They compare case evidence,
    approve or return each case, and leave an audit receipt.
  `;
  const review = recommendSurfaceTypes(selectedCaseDetailPaneArtifact);
...
{
  const explicitlySecondaryCentralArtifact = `
    ...
    secondary rendered interface remains central within its demo card, but it is
    not the primary object. Visitors may select a control and evidence attaches
    there. Completion is the demo request.
  `;
  const review = recommendSurfaceTypes(explicitlySecondaryCentralArtifact);
