...
Treat an exported PPTX as JudgmentKit output only when the MCP response or sidecar receipt confirms `tool_name: "mcp__judgmentkit.create_slide_deck"` and `deck_creation_status: "exported"`. A folder named `outputs/judgmentkit-slide-decks` is not provenance by itself.
...
4. Treat returned deck guidance or artifacts as JudgmentKit output only when the MCP response or sidecar receipt confirms `tool_name: "mcp__judgmentkit.create_slide_deck"` and `deck_creation_status: "exported"`.
