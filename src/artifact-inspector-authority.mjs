...
const DIAGNOSTIC_DEFAULTS = Object.freeze({
  ...
  JK_ARTIFACT_INSPECTOR_TRUSTED_RUNTIME_EVIDENCE_MISSING: Object.freeze({
    message:
      "The deferred trusted interactive attestation required for Artifact Inspector acceptance is unavailable in this release.",
    repair_instruction:
      "Keep the candidate at review_required until the separately released interactive-attestation contract can supply and verify the complete evidence matrix.",
  }),
});
...
export function normalizeArtifactInspectorBoundaryContracts(
  ...
  const normalized = value.map((entry, index) => {
    ...
    assertExactSet(
      roles,
      CANONICAL_BOUNDARY.allowed_roles,
      `${field}.allowed_roles`,
      "JK_ARTIFACT_INSPECTOR_BOUNDARY_EVENT_UNDECLARED",
    );
...
const DEFERRED_INTERACTIVE_ATTESTATION_OBSERVATIONS = Object.freeze([
...
// This requirement review is deliberately non-accepting. The current release
// defines the Artifact Inspector authority contract but has no trusted
// interactive-attestation producer or verifier. Extra caller arguments are
// ignored, so candidate-authored or pass-shaped evidence cannot alter the
// result.
export function reviewArtifactInspectorAuthorityEvidence(
  ...
  return {
    ...
    trusted_runtime_evidence: {
      ...
      static_browser_evidence_accepted: false,
      missing_observations: [
        ...DEFERRED_INTERACTIVE_ATTESTATION_OBSERVATIONS,
      ],
    },
    findings: stableFindings([
      missingRuntimeFinding({
        required_source: "deferred_interactive_attestation",
        candidate_authored_evidence_accepted: false,
        static_browser_evidence_accepted: false,
        missing_observations: [
          ...DEFERRED_INTERACTIVE_ATTESTATION_OBSERVATIONS,
