...
The homepage opens with one high-quality visual-composition video above the existing hero. A single native video element owns the picture, soundtrack, timeline, and loop for the entire 38.2-second replay; there is no live-renderer or audio-clock handoff. The video autoplays muted, and the three controls play or pause the video, scrub its timeline, and unmute or mute its soundtrack. The player selects distinct light or dark recordings and posters from the visitor's color-scheme preference while preserving the playhead and playback state when that preference changes. Native controls remain available until the custom controls bind successfully. The public player does not expose or publish a caption track; an associated screen-reader description summarizes the visual proof. The video is a replay of verified behavior, not proof that every policy shown is active on the public MCP. There is no separate visual-composition release or demo route.
